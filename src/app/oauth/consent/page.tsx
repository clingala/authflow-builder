...
export default async function OAuthConsentPage({ searchParams }: { searchParams: Promise<{ consent_challenge?: string }> }) {
  ...
  const consentChallenge: string = challenge;
  const { request, client, user } = await getOAuthPlatformService().consentContext(consentChallenge);

  async function completeConsent(decision: "accept" | "deny") {
    "use server";
    const service = getOAuthPlatformService();
    const redirectTo = decision === "accept" ? await service.acceptConsent(consentChallenge) : await service.rejectConsent(consentChallenge);
    redirect(redirectTo);
  }
  ...
  return <main className="oauth-consent-shell" style={{ background: client.configuration.branding.backgroundColor }}>
    <section className="oauth-consent-card">
      ...
      <form>
        <button className="button secondary" type="submit" formAction={completeConsent.bind(null, "deny")}>Deny</button>
        <button className="button primary" type="submit" formAction={completeConsent.bind(null, "accept")}>Allow</button>
      </form>
    </section>
  </main>;
}
